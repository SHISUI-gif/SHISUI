...
`my-genspark` is 志粋(Shisui) — a fully local AI assistant built on Ollama (Qwen models), with one deliberate exception: the "夜間修行" (autonomous study loop) calls the Gemini API as an external "mentor." Every other feature runs with zero cloud dependency except web search (Tavily + DuckDuckGo, see `src/research/web_search.py`) and Gemini (study loop only, requires `GEMINI_API_KEY`). `src/common/tools.py:execute_web_search()` queries both search providers and merges/dedupes by URL — either one can fail (missing/invalid Tavily key, DuckDuckGo rate-limited) without breaking the other; only returns "not found" if both come back empty.
