...
SLEEP_SYSTEM_PROMPT = (
    ...
    "「事実(fact)」は、ユーザー自身や、ユーザーと志粋の関係性についての事実に限る"
    "(例: ユーザーの状況・環境・過去の出来事)。志粋が雑学・スポーツの結果予想・"
    "時事的な話題などの質問に答えた「回答の内容そのもの」は、ユーザーについての\n"
    "情報ではないため、事実として記憶しないこと。\n"
...
AVATAR_SYSTEM_PROMPT = (
    ...
    "判定は寛容に行ってください: 話題の中心でなくても、ほんの一言でも・比喩や"
    "冗談としてでも触れていれば、関連する候補として積極的に含めてください。"
    "「完全に一致する話題だけ」に絞り込む必要はありません。迷ったら解除する側に"
    "倒してください(友達が全然コーデを解除できず、何日も同じ見た目のままに"
    "なってしまう方が問題です)。\n"
    "候補アイテム(スラッグ: 解除条件):\n{candidates}\n"
    "出力は必ず次のJSON配列形式のみとし、説明文やMarkdown装飾は付けないでください。\n"
    '["item_slug1", "item_slug2"]\n'
    "本当に何一つ関連する話題が無かった場合のみ、空配列 [] を返してください。"
)
